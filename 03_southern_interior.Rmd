---
title: Southern Interior Air Zone
header-includes:
  - \usepackage{booktabs}
  - \usepackage{makecell}
  - \usepackage{multirow}
...
```{r ozone-by-station, echo = FALSE, warn = FALSE, fig.cap = ozone_by_station_cap(airzone, ozone)}
plot_ozone_by_station(ozone, airzone)
```

<!-- Annual trends in ozone concentrations -->

```{r annual-ozone, echo = FALSE, warning = FALSE, fig.cap = annual_ozone_cap(annual_ozone)}
plot_ozone_station_timeseries(annual_ozone, airzone)
```

<!-- PM2.5 by station -->

```{r pm25-by-station, echo = FALSE, warning = FALSE, fig.asp = 1.3, fig.cap = pm25_by_station_cap(airzone, pm25)}
plot_pm25_by_station(pm25)
```

<!-- PM Annual trends in PM 2.5 -->

```{r annual-pm25, echo = FALSE, warning = FALSE, fig.cap = annual_pm25_cap(annual_pm25)}

<!--
NOTES:
     This is the main markdown page for the Air Zone Report
     Created: September 2023
     
     Instructions:
     > knit this markdown into html
     > change the html filename into "airzone_report.html"
     > open the html, edit and add the following: "<title>B.C. Air Zone Report</title>"
     > relocate the file into the IIS server that host the site
...
```
Air pollution is recognized as one of the greatest environmental threats to human health ([WHO 2022](https://www.who.int/news-room/fact-sheets/detail/ambient-(outdoor)-air-quality-and-health)). According to Health Canada estimates, approximately 15,300 Canadians--1,900 in British Columbia--die prematurely every year due to exposure to air pollution ([Health Canada 2021](https://www.canada.ca/en/health-canada/services/publications/healthy-living/2021-health-effects-indoor-air-pollution.html#a2)). In order to protect human health and guide work on air emissions, the federal, provincial, and territorial governments agreed to implement a national Air Quality Management System.
...
In this report, O<sub>3</sub> levels are presented using the [8-hour metric](## "Determined by selecting the 4th highest annual value from the highest daily 1-hour values obtained through the 8-hour rolling average measurements, and then averaging this annual value over a span of three consecutive years."). 
...
```
# On average, pollution levels in B.C. remain below the CAAQS values
...
```{=html}
 ...
 <p>
```
Pollutant concentrations have generally stayed below the thresholds set by the Canadian Ambient Air Quality Standards and have shown improving trends. However, PM<sub>2.5</sub> stands as an exception due to the rising frequency and intensity of wildfire smoke.
...
-   Ozone (O<sub>3</sub>) [8-hour metric](## "Determined by selecting the 4th highest annual value from the highest daily 1-hour values obtained through the 8-hour rolling average measurements, and then averaging this annual value over a span of three consecutive years."),`r o3_8h`% (since 1990)
...
-   Volatile Organic Compounds (VOC), `r voc_emission`% (since 1990). Ground-level ozone (O<sub>3</sub>) is formed from the complex reactions between VOC and NO<sub>x</sub>.
...
knitr::include_app('https://bcgov-env.shinyapps.io/airzonemanagementtrends/',height = "600px")
```

## <b>More information about management levels </b>
For more detailed information about the management levels of each air zones and monitoring stations, please refer to the provided links. <br>

   - [Click here](https://bcgov-env.shinyapps.io/airzonemanagementlevels_airzone/) to view the management levels of each air zones over the years.
  
  - [Click here](https://bcgov-env.shinyapps.io/airzonemanagementlevels_sites/) for the management levels of various locations in B.C. over the years
...
The interactive graphics below illustrate the trend of PM<sub>2.5</sub> levels in B.C. air zones. It shows the number of days when the air zones are exposed to PM<sub>2.5</sub> levels that are higher than the 24-hour value of the Canadian Ambient Air Quality Standard of 27 µg/m<sup>3</sup>. 

```{r, echo = FALSE,out.width="100%",out.height="700px"}
#located in \level_shiny\pm_exceedance
knitr::include_app('https://bcgov-env.shinyapps.io/pm_exceedance/',height = "700px")
```
...
pm_days_summary_totals <- pm_days_summary %>%
  group_by(AIRZONE,year) %>%
  summarize(total = sum(total,na.rm = TRUE),
            `wildfire days` = sum(`wildfire days`,na.rm = TRUE))

# -sort for the order of airzones based on PM and wildfire
...
# -adding details for the airzone with most days of high PM levels
mostdays_airzone <- order_pm25[1]
mostdays_totaldays <- pm_days_summary_totals$total[pm_days_summary_totals$AIRZONE == order_pm25[1]][1]
mostdays_wildfiredays <- pm_days_summary_totals$`wildfire days`[pm_days_summary_totals$AIRZONE == order_pm25[1]][1]

# -adding details for the airzone with most wildfires
mostwildfire_airzone <- order_wildfire[1]
mostwildfire_totaldays <-  pm_days_summary_totals$total[pm_days_summary_totals$AIRZONE == order_wildfire[1]][1]
mostwildfire_wildfiredays <-  pm_days_summary_totals$total[pm_days_summary_totals$AIRZONE == order_wildfire[1]][1]

```

## Quick Summary of Wildfire Smoke and PM<sub>2.5</sub> in `r validation_year`

-    British Columbia experienced `r BC_days_with_highpm` days with [high PM<sub>2.5</sub> levels](## "Days where the 24-hour value of the Canadian Ambient Air Quality Standards is exceeded. This means PM2.5 level is higher than 27 µg/m3"). Among these, `r BC_days_with_highpm_wildfire` days were caused by verified wildfire smoke, which occurs between summer and early fall.

-   The `r mostdays_airzone` air zone had the highest number of days with  [high PM<sub>2.5</sub> levels](## "Days where the 24-hour value of the Canadian Ambient Air Quality Standards is exceeded. This means PM2.5 level is higher than 27 µg/m3"). It experienced `r mostdays_totaldays` such days, out of which `r mostdays_wildfiredays` were due to verified wildfires, while the remainder came from other sources such as residential heating and open burning.

- The `r mostwildfire_airzone` air zone was the most affected by wildfire smoke, experiencing `r mostwildfire_wildfiredays` days with [high PM<sub>2.5</sub> levels](## "Days where the 24-hour value of the Canadian Ambient Air Quality Standards is exceeded. This means PM2.5 level is higher than 27 µg/m3") from verified wildfire smoke.
...
Click this link to download and share detailed management levels of each air zone and monitoring station: [Management Levels 2019-2021](https://envistaweb.env.gov.bc.ca/aqo/files/az_report/AirZoneReport_ManagementLevels_2021.pdf)

Click this link to download and share details of the transboundary flows and exceptional events adjustment that were made to the dataset: [Transboundary Flows and Exceptional Events 2021](https://envistaweb.env.gov.bc.ca/aqo/files/az_report/AirZoneReport_TFEE_20192021.pdf)

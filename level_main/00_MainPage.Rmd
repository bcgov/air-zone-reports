...
```{r, echo = FALSE,out.width="90%",out.height="700px"}
...
-    British Columbia experienced `r BC_days_with_highpm` days with [high PM<sub>2.5</sub> levels](## "Days where the 24-hour value of the Canadian Ambient Air Quality Standards is exceeded. This means PM2.5 level is higher than 27 µg/m3"). Among these, `r BC_days_with_highpm_wildfire` days were attributed to verified wildfire smoke, which commonly occurs between summer and early fall.

-   The `r order_wildfire[1]` air zone was the most affected by wildfire smoke, experiencing `r high_wildfire_daysnum` days with [high PM<sub>2.5</sub> levels](## "Days where the 24-hour value of the Canadian Ambient Air Quality Standards is exceeded. This means PM2.5 level is higher than 27 µg/m3") from verified wildfire smoke.

-   The `r order_pm25[1]` air zone had the highest number of days with  [high PM<sub>2.5</sub> levels](## "Days where the 24-hour value of the Canadian Ambient Air Quality Standards is exceeded. This means PM2.5 level is higher than 27 µg/m3"). It experienced `r high_airzone_pm` such days, out of which `r high_airzone_pm_wildfire` were due to wildfires, while the remainder came from sources such as residential heating and open burning.

...
`r calc_threshold(ozone_az$caaqs_ambient)` the national standard of `r achievement_level("o3")` ppb.
...
pmthresh = ifelse("above" %in% unique(pm25$threshold), "did not achieve", "achieved")

```


Between `r min(pm25$min_year, na.rm = TRUE)` and `r max(pm25$max_year, na.rm = TRUE)`, `r max(pm25$max_year, na.rm = TRUE)-min(pm25$min_year, na.rm = TRUE)` complete years of data were collected.  A 24-hour average value of `r pm24$metric_value_ambient` $\mu$g/m3 and an annual mean of `r pm_annual$metric_value_ambient` $\mu$g/m3 were obtained, indicating that PM~2.5~ levels at this site `r pmthresh` the national standards of `r achievement_level("pm2.5_24h")` and `r achievement_level("pm2.5_annual")` $\mu$g/m3, respectively. 

...
ozone_az <- filter(params$ozone_az, airzone == !!airzone)
...
```{r airzone-map, fig.width = 5, fig.cap = paste(airzone, "Air Zone")}
airzone_map(airzone)
```
...
`r min(ozone$metric_value_ambient, na.rm = TRUE)` to `r max(ozone$metric_value_ambient, na.rm = TRUE)` ppb. These levels were 
`r calc_threshold(ozone_az$caaqs_ambient)` the national standard of `r achievement_level("03")` ppb.

Trends in annual ozone levels are shown in Figure 3. Concentrations have remained `r calc_threshold(ozone_az$caaqs_ambient)` the level of the national standard over the period of record.

<!-- Ozone by station -->

```{r ozone-by-station, fig.width = 6, fig.cap = ozone_by_station_cap(airzone, ozone)}
...
pmthresh = ifelse("above" %in% unique(pm25$threshold), "above", "below")

```


Between `r min(pm25$min_year, na.rm = TRUE)` and `r max(pm25$max_year, na.rm = TRUE)`, `r max(pm25$max_year, na.rm = TRUE)-min(pm25$min_year, na.rm = TRUE)` complete years of data were collected.  A 24-hour average value of `r pm24$metric_value_ambient` g/m3 and an annual mean of `r pm_annual$metric_value_ambient` g/m3 were obtained,  indicating that PM~2.5~ levels at this site are `r pmthresh` those of the national standards of `r achievement_level("pm2.5_24h")` and `r achievement_level("pm2.5_annual")` g/m3, respectively. 
...
```{r pm25-by-station, fig.width = 6, fig.height=4, fig.cap = pm25_by_station_cap(airzone, pm25)}
plot_pm25_by_station(pm25) & expand_limits(x = 2, y = 2)
```
...
oz_col <- mgmt_colours$mgmt_col[mgmt_colours$mgmt_level == ozone_az$mgmt_level]

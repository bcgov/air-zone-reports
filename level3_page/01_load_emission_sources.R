...
df_npri <- readr::read_csv(paste(dirs_location,'NPRI.csv',sep='/'))
df_apei <-  readr::read_csv(paste(dirs_location,'EN_APEI-Can-Prov_Terr.csv',sep='/'))
...
plot_apei_complete <- function(df_apei = NULL) {
  ...
  plot_pollutants <- list(all_nodust = plot_nodust, all_dust = plot_overall2,pm25 = plot_pm25,nox=plot_nox,sox = plot_sox,voc = plot_voc)

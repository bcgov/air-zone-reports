...
ui <-
  ...
  (fluidPage(
             ...
             fluidRow(class = 'filters',
                      column(8,
                          ...
                          .irs-grid-text {font-size: 10px; color: white;}'
                             ...
                             selectizeInput('site',label = 'Select Monitoring Site:',
                                            choices = lst_sites,width = "5%")
                      )
    ...
    fluidRow(
       column(12,h6("Management Levels for the Location"),
...
server <- shinyServer(function(input, output) {
  ...
  observeEvent(input$site,
               {
                 print(paste('change site',input$site))
                 data <- mgmt_data$`all stations` %>%
                   filter(site == input$site)
                 
                 output$table2 <- function() {mgmt_data[[input$site]]}

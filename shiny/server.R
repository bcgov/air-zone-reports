...
server <-  function(input, output,session) {
  ...
  observe({
    ...
    if (!is.null(query[['panel01']])) {
      showTab(inputId = "navbar", target = "panel01")
    }
    if (!is.null(query[['panel02']])) {
      showTab(inputId = "navbar", target = "panel02")
    }
    if (!is.null(query[['panel03']])) {
      showTab(inputId = "navbar", target = "panel03")
    }
    if (!is.null(query[['panel04']])) {
      showTab(inputId = "navbar", target = "panel04")
    }
    if (!is.null(query[['panel05']])) {
      showTab(inputId = "navbar", target = "panel05")
    }
    if (!is.null(query[['panel06']])) {
      showTab(inputId = "navbar", target = "panel06")
    }
